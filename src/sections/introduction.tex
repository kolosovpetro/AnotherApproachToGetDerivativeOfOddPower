...
\end{align*}
These polynomials are obtained by rearranging the sums in the equation~\eqref{eq:definition-f} as
\[
...
The Interesting part is that odd-power function $g_{y} (x)$ may be obtained as a partial case of the function $f$,
as well as derivative of odd-power $\frac{d}{dx} g_{y}$ evaluated in point some $u\in\mathbb{R}$ may be obtained
as a sum of partial derivatives of $f$ evaluated in some fixed point, that is explained further in this manuscript.
...
\begin{example}
    ...
    (f_1)
        ...
        &=3 x - 6 z + 6 x z - 6 z^2
    \end{align*}
    Combining both $(f_1)^{'}_{x}$ and $(f_1)^{'}_{z}$ evaluated at the point $(u, u)$ we get
    \begin{align*}
    (f_1)
        ^{'}_{x} + (f_1)^{'}_{z}
        &= 3 x - 3 z + 6 x z - 3 z^2 \\
